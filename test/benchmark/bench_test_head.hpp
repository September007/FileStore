...
inline void Run_FileStore_Write(Context ctx, const vector<WOPE>& wopes) {
	...
	while (cnt < expect)this_thread::sleep_for(chrono::milliseconds(10));
}
...
inline vector<WOPE> create_test_WOPES(int sz,const unsigned int block_cnt=4,int block_data_length=3) {
	GHObject_t gh;
