...
// the default context is up to platform
    ...
    try {
        ...
        journalPath =
            filesystem::absolute(config["journalPath"].get<string>()).string();
        kvPath = filesystem::absolute(config["kvPath"].get<string>()).string();
        rbPath = filesystem::absolute(config["rbPath"].get<string>()).string();

        journal_callback_worker_count =
            config["journal_callback_worker_count"].get<int>();
        journal_write_worker_count =
            config["journal_write_worker_count"].get<int>();
        ...
        LOG_ERROR("context",
                  format("context load from [{}] failed,catch error[{}]", name,
                         e.what()));
        return false;
    }
}
