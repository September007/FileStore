...
public:
	...
	// repeat timer
	Timer(const TimerTask task, time_point when, Timer::clock::duration round, int times,
		const TimerAfter afterTask = nullptr)
		: task(task)
		, when(when)
		, caller(nullptr)
		, afterTask(afterTask)
	...
	Timer(const TimerTask task, time_point when, TimerCaller* caller = nullptr,
		const TimerAfter	   afterTask = nullptr,
		Timer::clock::duration round = chrono::milliseconds(0), int repeatation = 1)
		: task(task)
		, when(when)
		, caller(caller)
		, afterTask(afterTask)
...
template <typename T> class _MachineState {
protected:
...
public:
	...
	_MachineState(Timer::time_point::duration interval)
		: interval(interval)
	{
	}
	_MachineState(const _MachineState& m)
		: _shutdown(m._shutdown.load())
		, interval(m.interval)
	{
	}
	_MachineState(const _MachineState&& m)
...
public:
	TimerCaller(Timer::time_point::duration interval = default_interval)
		: _MachineState(interval)
	{
		// sentinel
...
public:
	TimerCallerGroup(Timer::clock::duration interval)
		: _MachineState(interval)
	{
	}
	TimerCallerGroup(const TimerCallerGroup& g)
		: _MachineState(*static_cast<const _MachineState*>(&g))
		, callers(g.callers)
		, access_to_callers()
	{
	}
	...
	auto AddCaller(
		string callerName, Timer::clock::duration interval = _MachineState::default_interval)
	{
...
template <typename T> inline void _MachineState<T>::run()
{
	while (is_actived()) {
...
/**
 * @brief this is concurency timer, name so because its timer task will be concurrent exexute in a
 * list-scan
 */
class ConcTimerCaller : public TimerCaller {
public:
	template <typename... args>
	ConcTimerCaller(int thread_count, args... ags)
		: tp(thread_count)
		, TimerCaller(std::forward<args>(ags)...)
	{
	}
	...
	void Tick() override
	{
		...
		for (auto it = timers.begin(); it != timers.end();) {
			...
			if (now >= it->when && is_actived()) {
				tp.enqueue([timer = move(*it)]() { timer.call(); });
				auto tmp = it++;
				timers.erase(tmp);
			} else
				break;
