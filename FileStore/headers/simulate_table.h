...
template <typename T> auto GetKey(T& t) { return t.GetKey(); }
/**
 * separate key and attr, this is access to attr
 */
template <typename T> auto GetAttr(T& t)
{
 ...
 */
template <TableType T> decay_t<T> GetAssemble(buffer& key, buffer& attr)
{
	using RT = decay_t<T>;
	uint8_t b[sizeof(RT)];
...
template <TableType T> using TableKeyType = decltype(GetKey(declval<T&>()));

template <TableType T> using TableAttrType = decltype(GetAttr(declval<T&>()));
