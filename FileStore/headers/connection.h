...
enum class ConnectionReturnType { success = 0, fail_of_primary, fail_of_replica, fail_anyway };
class InfoForOSD {
public:
	string name;
	string host;
	int	   port;
	InfoForOSD(const string& name = "", const string& host = "", int port = 0)
		: name(name)
		, host(host)
		, port(port)
	{
	}
	bool operator==(const InfoForOSD& ifs) const
	{
		return name == ifs.name && host == ifs.host && port == ifs.port;
	}
...
enum class reqType {
	Write,	 // osd distinguish either it's as primary osd or not by mathcing it's info in incoming
			 // param tos,if it's on first,then it's primary
	Read,
...
enum class WOpeState { other, failed, onJournal, onDisk };
enum class ROpeState { other, failed, success };
union OpeState {
	WOpeState wope;
	ROpeState rope;
};
bool http_send(
	InfoForNetNode from, InfoForNetNode to, buffer data, const string& postname = "/http_send");
// bool WriteReq(InfoForNetNode from, vector<InfoForNetNode> tos, buffer data, reqType rt,opeIdType
// oid); bool WriteRep(InfoForNetNode from, vector<InfoForNetNode> tos, buffer data, repType
// rt,opeIdType oid);
