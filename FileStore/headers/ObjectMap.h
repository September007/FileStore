...
public:
	...
	template<typename KeyType, typename ValType>
	ValType Read_Meta(const KeyType key) {
		//performance
		buffer buf;
...
template<TableType T>
inline void ObjectMap::Write_Meta(T& t) {
...
*/
template<TableType T>
T ObjectMap::Read_Meta(const TableKeyType<T>& key) {
...
template<TableType T>
T ObjectMap::Read_Meta(const T& key) {
