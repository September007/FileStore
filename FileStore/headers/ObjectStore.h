...
protected:
  ...
  ReferedBlock addNewReferedBlock(string data, string root_path) {
    ...
    LOG_INFO("rb_log", fmt::format("add new rb[{} ref:{} ,path:{}]", rb.serial,
                                   rb.refer_count,
  ...
  inline void WriteReferedBlock(ReferedBlock rb, string root_path,
                                string data) {
    auto path = GetReferedBlockStoragePath(rb, root_path);
    ctx->m_WriteFile(path, data, true);
  }
  inline string GetReferedBlockStoragePath(const ReferedBlock &rb,
                                           string root_path) {
...
public:
  static ObjectStore *create(const string &type, const string &path,
                             const string &journal_path, const string &kv_path);
   ...
   */
  virtual void Modify(const GHObject_t &ghobj, int modifiedArea_start, int len,
                      const string &data) = 0;
};
